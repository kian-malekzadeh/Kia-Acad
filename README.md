# Kia Academy (کیا آکادمی) — Adaptive Learning Platform
...
**Live frontend (GitHub Pages):** https://kian-malekzadeh.github.io/Kia-Acad/

> GitHub project Pages always use `https://<user>.github.io/<repository-name>/`.
> This GitHub repository is named **`Kia-Acad`**, so the live URL path is `/Kia-Acad/`.
> If you fork or rename the repository, update `basePath` accordingly — pass
> `PAGES_REPO_NAME=<your-repo-name>` for local `pnpm build:pages`, and the
> **Deploy GitHub Pages** workflow sets `basePath` from the repo name automatically.

## GitHub Pages

The web app is statically exported with `basePath` matching the GitHub repository name
(currently `/Kia-Acad` → https://kian-malekzadeh.github.io/Kia-Acad/).
...
4. Save, wait ~1 minute, then open https://kian-malekzadeh.github.io/Kia-Acad/
...
```bash
pnpm build:pages
# Output: apps/web/out  (asset URLs expect /Kia-Acad/ — set PAGES_REPO_NAME to override after renaming the repo)
```
