...
#
# Kia Academy — multi-stage build for API (NestJS) + Web (Next.js standalone).
# Node 22 LTS to match .nvmrc and engines (">=22.13").
...
# ---- Dependencies (with native module toolchain: bcrypt, prisma engines) ----
FROM base AS deps
RUN apt-get update \
...
# Prisma schema is required during install: root postinstall runs `prisma generate`
